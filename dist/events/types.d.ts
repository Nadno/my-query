 ...
 */
export type Handler<Ev extends Event = Event, E extends Element = Element> = (event: Ev, ctx: MQ<E>) => void;
/**
 ...
 * `$onMounted(() => () => cleanup)`.
 */
export type PairedHandler<Ev extends Event = Event, E extends Element = Element> = (event: Ev, ctx: MQ<E>) => void | Cleanup;
export type Modifier<Ev extends Event = Event> = (next: Handler<Ev>) => Handler<Ev>;
 ...
 */
export type OnValue<Ev extends Event = Event, E extends Element = Element, H extends (event: Ev, ctx: MQ<E>) => unknown = Handler<Ev, E>, Opts = AddEventListenerOptions> = H | readonly [H, ...(Modifier<Ev> | Opts)[]];
...
export type OnMap<E extends Element = Element> = {
    ...
    [K in keyof MQCustomEventMap]?: OnValue<MQCustomEventMap[K], E, PairedHandler<MQCustomEventMap[K], E>, MQCustomEventOptions[K]>;
} & {
