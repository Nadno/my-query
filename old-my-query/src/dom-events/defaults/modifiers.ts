...
import { ModifierFactory } from '../modifiers';
...
export const captureFactory: EventHandlerModifierFactory = function capture(
  ...
  return {
    identifier: 'capture',
    handler,
    meta: { target, options: { ...options, capture: true } },
  };
};
...
export const onceFactory: EventHandlerModifierFactory = function once(
  ...
  return {
    ...
    meta: {
      target,
      options: end ? options : { ...options, once: true },
    },
  };
};
      ...
      handler: resolveHandler(
        ...
        (handler) =>
          (e: Event, ...args: any[]) => {
            ...
            if (
              !$target ||
              (!$target.matches(delegatedTarget) && !$target.closest(delegatedTarget))
            )
              return;
