...
import { DirectiveClass, isDirectiveClass, isReactiveSource } from './directive';
...
export class QueryDirective<T extends Element> implements IQueryDirective<T> {
  ...
  private _handleReactive(
    ...
    if (reactive.getValue && directiveInstance.instance.update) {
      directiveInstance.instance.update(reactive.getValue());
    }
