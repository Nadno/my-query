...
export declare const STYLE_HANDLE: unique symbol;
/** Marca um elemento como teleportado — `appendChild` não o anexa ao pai. */
export declare const TELEPORTED: unique symbol;
...
export type ClassValue = string | number | false | null | undefined | ClassHandle | ClassValue[] | Record<string, boolean | null | undefined>;
...
export type Props<T extends TagName> = StaticAttrs<T> & ReactiveAttrs<T> & {
    class?: ClassValue;
...
export type SetupFn<T extends TagName, P = Record<string, unknown>> = (props: P, ctx: MQ<TagElement<T>>) => unknown;
/**
 ...
 */
export type NonEmptyFn<F> = F extends (...args: infer A) => unknown ? A extends readonly [] ? never : F : never;
/** Filho renderizável. */
export type Child = Node | string | number | boolean | null | undefined | Child[] | ComponentTuple | (() => unknown) | {
