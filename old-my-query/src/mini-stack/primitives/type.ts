...
const _assert = Object.assign(
  ...
  },
  {
    string(v: unknown, msg?: string): asserts v is string {
      if (!isString(v))
        throw new Error(msg ?? `Expected string, got ${_tag(v)}`);
    },
    number(v: unknown, msg?: string): asserts v is number {
      if (!isNumber(v))
        throw new Error(msg ?? `Expected number, got ${_tag(v)}`);
    },
    boolean(v: unknown, msg?: string): asserts v is boolean {
      if (!isBoolean(v))
        throw new Error(msg ?? `Expected boolean, got ${_tag(v)}`);
    },
    function(
      v: unknown,
      msg?: string,
    ): asserts v is (...args: unknown[]) => unknown {
      if (!isFunction(v))
        throw new Error(msg ?? `Expected function, got ${_tag(v)}`);
    },
    ...
    object(v: unknown, msg?: string): asserts v is Record<string, unknown> {
      if (!isObject(v))
        throw new Error(msg ?? `Expected object, got ${_tag(v)}`);
    },
  },
);
