...
type ResolvedParts<T extends StyleConfig> = (T extends {
    ...
    [K in keyof T as K extends `>${infer R}` ? R : never]: T[K] extends StyleConfig ? T[K] : StyleConfig;
} : {});
...
interface StyleHandleBase<T extends StyleConfig> {
    ...
    readonly flags: {
        [K in keyof T['flags']]: string;
    };
    /** Token de cada variante (`variants.size.sm === '--size-sm'`). */
    readonly variants: {
        [G in keyof T['variants']]: {
            [V in keyof NonNullable<T['variants']>[G]]: string;
        };
    };
    /** Nome escopado de cada keyframe (`'field-pulse'`). */
    readonly keyframes: {
        [K in keyof T['keyframes']]: string;
    };
